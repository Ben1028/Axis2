...
DWORD CButtonST::SetBGColor(COLORREF crColor, BOOL bRepaint)
{

	// Set new color
	m_crColors[0] = crColor;
	m_crColors[2] = crColor;
	m_crColors[4] = crColor;
...
DWORD CButtonST::CreateMenu(HWND hParentWnd, BOOL bRepaint)
{
	// Destroy any previous menu
	if (m_hMenu)
	{
	...
	::AppendMenu(m_hMenu, MF_POPUP|MF_STRING, (UINT_PTR) hSub , _T("MENU"));
	m_hParentWndMenu = hParentWnd;
...
void CButtonST::InitButton(int nIcon, CString csTip, UINT nMenu, HWND hParentWnd)
{
	SetIcon(nIcon,(int)BTNST_AUTO_DARKER);
	DrawBorder(false);
	//EnableBalloonTooltip();
	SetTooltipText(csTip);
	SetMenu(nMenu, hParentWnd);
}