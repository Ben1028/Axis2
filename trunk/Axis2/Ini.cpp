...
DWORD CIni::GetString(LPCTSTR lpSection, LPCTSTR lpKey, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDefault) const
{
	...
	if (lpBuffer != NULL)
	{
		_tcsncpy_s(lpBuffer, dwLen, psz, dwBufSize);
...
{
	UNREFERENCED_PARAMETER(bTrimString);
	if (pArray != NULL)
		pArray->RemoveAll();
...
//#ifdef __AFXWIN_H__
// Retrieve a list of key-pairs of the specified section
void CIni::GetKeyArray(LPCTSTR lpSection, CAdvStringArray *pArray) const
{
	if (pArray != NULL)
		pArray->RemoveAll();
	...
	LPTSTR psz = new TCHAR[LEN + 1];
	GetKeyLines(lpSection, psz, LEN);
	ParseDNTString(psz, __SubStrInsert, pArray);
	delete [] psz;
}
//#endif

//#ifdef __AFXWIN_H__
BOOL CALLBACK CIni::__SubStrInsert(LPCTSTR lpString, LPVOID lpParam)
{
	CAdvStringArray* pArray = (CAdvStringArray*)lpParam;
	if (pArray == NULL || lpString == NULL)
		return FALSE;

	pArray->Insert(lpString);
	return TRUE;
}
//#endif
