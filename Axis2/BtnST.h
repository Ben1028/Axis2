...
public:
	...
	DWORD SetBGColor(COLORREF crColor, BOOL bRepaint = TRUE);
	...
	void InitButton(int nIcon, CString csTip = NULL, UINT nMenu = NULL, HWND hParentWnd = NULL);

	HMENU GetMenu() {return ::GetSubMenu(m_hMenu, 0);}
	HMENU GetSubMenu(UINT uMenu = 0) {HMENU hSubMenu = ::GetSubMenu(GetMenu(), uMenu); return hSubMenu;}
	...
	DWORD CreateMenu(HWND hParentWnd, BOOL bRepaint = TRUE);
#endif
